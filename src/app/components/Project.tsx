...
export const Project = ({ title, description, language, stacks, url, status, image }: ProjectProps) => {
    return (

        <div className="border-[#27272a] border rounded-xl w-96 p-4">
            {
                image && <img src={image} alt="Imagem" className="w-full h-40 object-cover mb-4"/>
            }
            <Link href={url}>
            ...
            <div className="flex gap-4 mt-4">
                {
                    stacks.map((stack, index) => {
                        return (<p key={index} className="bg-primary px-2 py-1 rounded-lg text-sm font-medium">{stack}</p>)
                    })
                }
            </div>
        </div>
